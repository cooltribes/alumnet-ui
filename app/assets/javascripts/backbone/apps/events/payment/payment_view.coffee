@AlumNet.module 'EventsApp.Payment', (Payment, @AlumNet, Backbone, Marionette, $, _) ->
  ...
  class Payment.PaymentView extends Marionette.ItemView
    ...
    reloadWidget: (e)->
      ...
      country.fetch
        success: (model) ->
          parameters_string = 'ag_external_id=order_no_555123ag_name='+view.model.get("name")+'ag_type=fixedamount='+price+'country_code='+model.get('cc_fips')+'currencyCode=USDkey='+paymentwall_project_key+'sign_version=2success_url='+paymentwall_return_url+'uid='+view.current_user.get("id")+'widget=p1_1ea9c9cad7ce7d4c6ad745b48f36a9d45'
